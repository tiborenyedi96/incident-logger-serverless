  ...
  policy = jsonencode({
    ...
    Statement = [
      ...
      },
      {
        ...
        Resource : "arn:aws:dynamodb:${data.aws_region.current.id}:${data.aws_caller_identity.current.account_id}:table/incident-logger-tf-lock"
      },
      {
        Sid : "ReadOnlyForPlan",
        Effect : "Allow",
    ...
    Statement = [
      {
        ...
        Resource = "arn:aws:lambda:${data.aws_region.current.id}:${data.aws_caller_identity.current.account_id}:function:incident-logger-*"
      }
    ]
  })
}

resource "aws_iam_role_policy_attachment" "lambda_ecr_power_user" {
